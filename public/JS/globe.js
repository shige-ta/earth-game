...
function animate() {
    ...
    // document.querySelector("#x").innerHTML = camera.position.x
    // document.querySelector("#y").innerHTML = camera.position.y
...
window.addEventListener("mousemove", function(event) {
    ...
    if(event.movementX < 0 && mouseposi == true){
        let elm = document.getElementById("jquery-ui-draggable");
    ...
    document.querySelector("#x").innerHTML = camera.position.x;
    document.querySelector("#count").innerHTML = count + "回";
    // if(Math.sign(camera.position.x) == -1 && (plusflg == false))
    // {
    //     plusflg = true;
    //     count++;
    // }
    // if(Math.sign(camera.position.x) == 1 && (plusflg == true))
    // {
    //     plusflg = false;
    // }
    ...
    if( startflg == true && Math.sign(saveXnumber) - Math.sign(camera.position.x) != 0 && event.movementX > 0 && (minusflg == false))
    {
        minusflg = true;
        count++;
    }
    if(Math.sign(camera.position.x) <= -1 && (minusflg == true))
    {
        minusflg = false;
    }

    saveXnumber = camera.position.x

    // document.querySelector("#y").innerHTML = ;

    // if( (event.movementX > 0) && (camera.position.x < 0))
    // {
    //     count++;
    //     flg == true;
    // } 
